...
# TODO: This file was copied from Robotframework, needs cleanup.
"""
...
VERSION_FILE = os.path.join('src', 'Selenium2Library', 'version.py')
...
def set_version(version, push=False):
    """Set version in `src/Selenium2Library/version.py`.
...
def copy_robot_files(build_dir='build'):
    source = os.path.join('src', 'Selenium2Library')
    target = os.path.join(build_dir, 'Lib', 'Selenium2Library')
...
def compile_python_files(jython_jar, build_dir='build'):
    ...
    # Jython will not work without its py-files, but Selenium2Library will
    for directory, _, files in os.walk(os.path.join(build_dir, 'Lib', 'Selenium2Library')):
        for name in files:
            if name.endswith('.py'):
                os.remove(os.path.join(directory, name))
...
def create_robot_jar(version, source='build'):
    ...
    target = os.path.join('dist', 'robotframework-selenium2library-{0}.jar'.format(version))
...
Main-Class: org.robotframework.Selenium2Library
